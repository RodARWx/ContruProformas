...
# --- E8: categoría con rubros ---
Test-EdgeCase "E8" "No eliminar categoría con rubros (409)" {
    ...
    if (-not $withRubros) {
        Write-Host "  (omitido: no hay categorías con rubros)" -ForegroundColor DarkYellow
        return
    }
...
# --- E7: búsqueda clientes con límite ---
...
Write-Host ""
Write-Host "Resumen: $passed OK, $failed FALLOS" -ForegroundColor $(if ($failed -eq 0) { "Green" } else { "Red" })
